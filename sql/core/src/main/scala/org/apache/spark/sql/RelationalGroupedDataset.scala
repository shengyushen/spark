 ...
 */
@Stable
class RelationalGroupedDataset protected[sql]( // SSY handling group* operators
    private[sql] val df: DataFrame,
    private[sql] val groupingExprs: Seq[Expression],
    groupType: RelationalGroupedDataset.GroupType) {
	//SSY this will execute in toDF
  private[this] def toDF(aggExprs: Seq[Expression]): DataFrame = {
    val aggregates = if (df.sparkSession.sessionState.conf.dataFrameRetainGroupColumns) {
      groupingExprs ++ aggExprs
    } else {
      aggExprs
    }// SSY still only generate Seq instead of evaluation

    val aliasedAgg = aggregates.map(alias)
		// SSY this will execute in ofRows
    groupType match {
      case RelationalGroupedDataset.GroupByType => // SSY generated by Dataset groupBy
				// SSY ofRows will execute the logicalPlan and then wrap the result in Dataset
        Dataset.ofRows(df.sparkSession, Aggregate(groupingExprs, aliasedAgg, df.logicalPlan)) // SSY Aggregate defined in sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/plans/logical/basicLogicalOperators.scala
   ...
   * @since 1.3.0
   */
	// SSY sql/core/src/main/scala/org/apache/spark/sql/package.scala Dataset
	// SSY sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/expressions/literals.scala generate Literal extending LeafExpression extending Expression
	// SSY sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/expressions/aggregate/Count.scala Count input Seq Expression and generate DeclarativeAggregate
	// SSY toDF actually process all inside its () anf get a Dataset wraping result
